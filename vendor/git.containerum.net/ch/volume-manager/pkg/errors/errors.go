...
func ErrDownResize(params ...func(*cherry.Err)) *cherry.Err {
	err := &cherry.Err{Message: "Can`t resize volume to lower capacity", StatusHTTP: 400, ID: cherry.ErrID{SID: "volume-manager", Kind: 0xb}, Details: []string(nil), Fields: cherry.Fields(nil)}
	for _, param := range params {
		param(err)
	}
	for i, detail := range err.Details {
